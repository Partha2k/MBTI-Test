...
def calc_result_TF(name):
    ...
    for i in range(len(q_wgt_b)):
        sum_one_b += f_one_b[i]*q_wgt_b[i][0]
    if sum_one_a > sum_one_b:
       return p_type[4]+" T score "+str(sum_one_a)+" F Score "+str(sum_one_b)
    else:
       return p_type[5]+" T score "+str(sum_one_a)+" F Score "+str(sum_one_b)
...
def calc_result_JP(name):
    ...
    for i in range(len(q_wgt_b)):
        sum_one_b += f_one_b[i]*q_wgt_b[i][0]
    if sum_one_a > sum_one_b:
       return p_type[6]+" J score "+str(sum_one_a)+" P Score "+str(sum_one_b)
    else:
       return p_type[7]+" J score "+str(sum_one_a)+" P Score "+str(sum_one_b)
...
def test_result():
    if 'user' in session:
       ...
       print type_four
       print 10*'#'
       return render_template('result.html', EorI = type_one, SorN = type_two, TorF = type_three, JorP = type_four)
    else:
       flash("You are not logged in.")
       return redirect(url_for('index'))
